...
- 📐 **도메인 규칙 & 분석 알고리즘 명세서**: [docs/domain_rules.md](docs/domain_rules.md)
- 🏗️ **모듈화 & 리팩토링 아키텍처 청사진**: [docs/architecture_refactoring.md](docs/architecture_refactoring.md)
...
| `python analyze.py`                                | `data/records.csv`, `data/athlete_info.csv` (필수) / `data/athlete_index.csv`(선택, `SPLITS_ATHLETE_INDEX_CSV`로 경로 지정 가능), `data/id_merges.csv`(선택, 기존 병합표 재사용), `data/records_full.csv` + `data/athlete_info_full.csv`(선택, 익명 통계에만 우선 사용)                                                             | `data/clean_records.csv`, `data/placements.csv`, `data/youth_summary.csv`, `data/outliers.csv`, `data/coverage.csv`, `data/age_matrix.csv`, `data/best_heat_times.csv`, `data/school_raw_list.txt`, `data/school_aliases.csv`, `data/school_ambiguous.csv`, `data/id_merge_candidates.csv`, `data/id_merges.csv`, `data/stats_distribution.csv`, `data/stats_participation.csv`, `data/season_month_histogram.csv`, `data/participation.csv`, `data/season_activity_counts.csv`, `data/class_transition_summary.csv`, `data/gap_return_rates.csv`, `data/record_stop_rule.csv`, `data/cohort.csv`, `data/cohort_stage_summary.csv`, `data/retention_overall.csv`, `data/retention_band.csv`, `data/improvement_rate.csv`, `data/reverse_distribution.csv`, `analysis/retention_overall.svg` | `build_data.py`, `collect_full_history.py(간접: id_merges)` |
| `python -m rating.ledger.graph --results <path>`   | 정제·익명화 경기결과 CSV (`race_id` 권장, 없으면 복원 시도). 예: `data/records_anon.csv`                                                                                     | `out/connectivity_report.md`, `docs/adr/0001-rating-feasibility.md`                                                                                                                                                                                                                                                                                 | R-02 레이팅 계산 착수/보류 판정                              |
| `python -m rating.ledger.extract --results <path> --policy <preset> --out <parquet>` | 경기결과 CSV(기본 권장: `data/records_full.csv`, status 코드 감사 필요)                                                                 | `out/status_audit.md`, `<out parquet>`                                                                                                                                                                                                                                                                                                                | R-05 정책 백테스트 입력 비교 데이터 생성                     |
| `python -m rating.ledger.build --results <path> --policy <preset> --out <dir>` | 경기결과 CSV(기본 권장: `data/records_anon.csv` 또는 `data/records_full.csv`)                                                                 | `out/ledger/race_ledger/season=YYYY/part.parquet`, `out/ledger/pairwise_view/season=YYYY/part.parquet`, `out/ledger/ranking_view/season=YYYY/part.parquet`                                                                                                                                                                                          | R-04 레이팅 리플레이 입력 레저 생성                          |
| `python -m rating.ledger.schema --validate <out/ledger 또는 race_ledger 경로>` | `rating.ledger.build` 산출물 (`race_ledger` parquet)                                                                                                                               | 추가 파일 생성 없음(검증 결과 로그 출력)                                                                                                                                                                                                                                                                                                               | 레저 스키마/결정성 회귀 점검                                  |
| `python -m rating.engine.runner --ledger out/ledger --out out/ratings_baseline.parquet --report out/baseline_report.md --engine glicko2 --rating-period meet` | `out/ledger/race_ledger` (R-03 산출물)                                                                                                                                           | `out/ratings_baseline.parquet`, `out/baseline_report.md`                                                                                                                                                                                                                                                                                             | R-05 백테스트 입력 베이스라인 스냅샷 생성                     |
| `python -m rating.eval.backtest --ledger out/ledger --holdout-seasons 2 --all-configs --out out/backtest_report.md` | `out/ledger/<policy>/race_ledger` (`conservative/aggressive/place_only`) 또는 단일 `out/ledger/race_ledger` | `out/backtest_report.md`, `out/backtest_calibration/*.svg`, `docs/adr/0006-backtest-verdict-rerun.md` | R-05 GO/STOP 판정 및 정책/period/엔진 선택 근거 |
...
| `python build_data.py`                             | `data/placements.csv`, `data/youth_summary.csv`, `data/age_matrix.csv`, `data/athlete_info.csv`, `data/coverage.csv`, `data/public_figures.csv`, `data/stats_distribution.csv` + (선택) `data/reverse_distribution.csv` + **대회 집계용 원천 1쌍 필수**: `records_full.csv`+`athlete_info_full.csv`(1순위) 또는 `records.csv`+`athlete_info.csv`(2순위), 환경변수 `SPLITS_ANON_SALT` (선택: `SPLITS_MEET_INDEX_CSV`) | `data/records_anon.csv`, `site/data/athletes.json`, `site/data/meets.json`, `site/data/distribution.json`, `site/data/reverse_distribution.json`, `site/data/meta.json`                                                                                                                                                                              | `build_site.py`                                             |
...
| `data/retention_overall.csv`                           | 전체 지속률 기준선(학년별 잔존/신규진입/이탈률 + 단계 도달률)   |
| `data/retention_band.csv`                              | 초5~6 성적 구간별(기록/순위 기준) 중등·고등·대학 도달 교차표     |
| `data/improvement_rate.csv`                             | 학년·성별·거리별 향상 속도 기준선과 구간별 비교/예측력 비교      |
| `data/reverse_distribution.csv`                        | 도달 단계별(대학·일반/고등 완주/국가대표) 유년기 백분위·유형 분포 |
| `out/connectivity_report.md`                           | R-01 비교 그래프 연결성 지표 리포트(거대 성분/다리 강도/GO·STOP 근거) |
| `docs/adr/0001-rating-feasibility.md`                 | R-01 판정 규칙 기반 최종 의사결정( GO / 조건부 GO / STOP ) |
| `out/status_audit.md`                                 | R-02 원천 status 코드 전수 감사표(빈도/희귀코드/처리방침) |
| `docs/adr/0002-comparison-extraction.md`              | R-02 실격·라운드 정책 프리셋과 비교 수 산출 근거 |
| `site/data/*.json`                                     | Astro 빌드 계약 데이터                                         |
| `index.html`, `athlete/`, `meet/` 등                   | 최종 배포 정적 산출물                                          |

## 6) 운영 원칙

- `data/` 커밋 허용 파일은 `records_anon.csv`, `stats_distribution.csv`, `stats_participation.csv`, `season_month_histogram.csv`, `participation.csv`, `season_activity_counts.csv`, `class_transition_summary.csv`, `gap_return_rates.csv`, `record_stop_rule.csv`, `cohort.csv`, `cohort_stage_summary.csv`, `retention_overall.csv`, `retention_band.csv`, `improvement_rate.csv`, `reverse_distribution.csv`, `public_figures.csv`, `coverage.csv`, `meet_index_inf201.csv`(및 `data/README.md`)로 제한합니다.
...
- `python analyze.py`는 `records.csv`/`athlete_info.csv`가 없고 `records_anon.csv`만 있는 경우에도 `coverage.csv`, `stats_distribution.csv`, `stats_participation.csv`, `season_month_histogram.csv`, `participation.csv`, `season_activity_counts.csv`, `class_transition_summary.csv`, `gap_return_rates.csv`, `record_stop_rule.csv`, `cohort.csv`, `cohort_stage_summary.csv`, `retention_overall.csv`, `retention_band.csv`, `improvement_rate.csv`, `reverse_distribution.csv`를 재생성할 수 있습니다.
