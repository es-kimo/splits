...
flowchart TD
  ...
  M --> P[data/raw/inf201|inf202|inf301|inf310|inf503]
  M --> Q[data/collect_progress.json + data/collect_failures.csv]
  L -. 소속 집계 보조 입력 .-> E
...
| `python collect_full_history.py collect`           | 네트워크 원천(INF201→INF202→INF301→INF310), 기존 캐시/진행상태(`data/raw/**`, `data/collect_progress.json`)                                                                    | `data/raw/inf201/{page}.html`, `data/raw/inf202/{classCd}_{toCd}.html`, `data/raw/inf301/{toCd}_{kindCd}_{detailClassCd}.html`, `data/raw/inf310/{toCd}_{kindCd}_{detailClassCd}_{baseClassCd}_{rhCd}_{pcntGbn}.html`, `data/raw/inf503/{idNo}.html`, `data/collect_progress.json`, `data/collect_failures.csv`, `data/inf202_empty_events.csv`, `data/records_full.csv`, `data/athlete_info_full.csv` | `analyze.py` 익명 통계 입력 우선, `build_data.py` 대회 집계 입력 우선 |
| `python collect_full_history.py retry-failures`    | `data/collect_failures.csv`                                                                                                                                                    | 위 `collect`와 동일 파일 갱신                                                                                                                                                                                                                                                                                                                       | 동일                                                        |
| `python collect_full_history.py export`            | `data/raw/inf201|inf202|inf301|inf310|inf503` 캐시                                                                                                                              | `data/records_full.csv`, `data/athlete_info_full.csv` 재생성                                                                                                                                                                                                                                                                                        | 동일                                                        |
...
- `retry-failures`: 실패 목록 재시도 후 캐시 기반 누락 요청 이어받기 + full CSV 재생성
...
- `collect`는 classCd=2(쇼트트랙) 대회 전수 수집 후, 대회명에 `동계체`가 포함된 참가자만 `INF503`를 보완 조회해 `채점종합`·출생년도를 채웁니다.
