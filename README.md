...
| `python collect_full_history.py collect`           | 네트워크 원천(INF201→INF301(kind옵션)→detailClass AJAX→INF301→INF310), 기존 캐시/진행상태(`data/raw/**`, `data/collect_progress.json`)                                       | `data/raw/inf201/{page}.html`, `data/raw/inf301_kind/{toCd}.html`, `data/raw/detail_class_ajax/{classCd}_{toCd}_{kindCd}.json`, `data/raw/inf301/{toCd}_{kindCd}_{detailClassCd}.html`, `data/raw/inf310/{toCd}_{kindCd}_{detailClassCd}_{baseClassCd}_{rhCd}_{pcntGbn}.html`, `data/raw/inf503/{idNo}.html`, `data/collect_progress.json`, `data/collect_failures.csv`, `data/detail_failures.csv`, `data/collect_request_failures.csv`, `data/suspicious_ids.csv`, `data/records_full.csv`, `data/athlete_info_full.csv` | `analyze.py` 익명 통계 입력 우선, `build_data.py` 대회 집계 입력 우선 |
