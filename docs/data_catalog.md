...
| `out/rating_runs/runs/<run_id>/rating_run.json`  | JSON     | 결정적 `run_id`, 입력 스냅샷 ID, 보정기, 보정 전/후 확률 digest를 담은 실행 manifest             |
...
| `out/ledger/*.parquet`                       | `records_anon.csv`(기본) 또는 `records_full.csv`                                     | `python -m rating.ledger.build`                       | `rating.engine.runner`                   |
